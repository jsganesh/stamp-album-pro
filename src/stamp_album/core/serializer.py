...
import dataclasses
from stamp_uma.core.models import Album, Page, Row, Stamp
...
class AlbumSerializer:
    ...
    def to_dsl(self, album: Album) -> str:
        lines = []
        if album.title: lines.append(f'ALBUM_TITLE("{album.title}")')
        if album.author: lines.append(f'ALBUM_AUTHOR("{album.author}")')
        ps = album.page_setup
